...
# get or write game folder location
...
class ModManager(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Project Rebearth Texture Workshop v1.0")
    ...
    def browse_game(self):
        path = filedialog.askdirectory(title="Select Game Steam Folder")
        if path:
            self.game_path = path
            self.res_path = os.path.join(path, "resources")
            self.app_folder = os.path.join(self.res_path, "app")
        ...
        def task(prog):
            ...
            for item in os.listdir(self.res_path):
                if item.startswith("app.asar") and item != "app" and item != "_backups":
                    shutil.move(os.path.join(self.res_path, item), os.path.join(backups, item))
            p_src = os.path.join(os.getcwd(), "assets", "electron.exe")
            if os.path.exists(p_src): shutil.copy2(p_src, os.path.join(self.game_path, "project_rebearth.exe"))
    ...
    def apply_selected_pack(self):
        ...
        def task(prog):
            # copy target data to game folder
